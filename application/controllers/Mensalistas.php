...
class Mensalistas extends CI_Controller {
    ...
    public function core($mensalista_id = NULL) {
        if (!$mensalista_id) {
            ...
            $this->form_validation->set_rules('mensalista_rg', 'RG', 'trim|required|max_length[20]|is_unique[mensalistas.mensalista_rg]');
            ...
            $this->form_validation->set_rules('mensalista_dia_vencimento', 'Dia Vencimento Mensalidade', 'trim|integer|greater_than[0]|less_than[32]'); // SE QUISER QUE SEJA ATÉ 28 POR CONTA DE FEVEREIRO MUDAR PARA: less_than[29] 
            ...
            if (!$this->core_model->get_by_id('mensalistas', array('mensalista_id' => $mensalista_id))) {
                ...
                $this->form_validation->set_rules('mensalista_rg', 'RG', 'trim|required|max_length[20]|callback_check_rg');
                    ...
                    if ($mensalista_ativo == 0) {
                        if ($this->db->table_exists('mensalidades')) {
                            if ($this->core_model->get_by_id('mensalidades', array('mensalidade_mensalista_id' => $mensalista_id))) {
                                $this->session->set_flashdata('error', 'Não é possível desativar esse mensalista, pois mesmo está atrelado á <i class="fas fa-hand-holding-usd"></i>&nbsp;Mensalidades');
                                redirect($this->router->fetch_class());
                            }
                        }
                    }
    ...
    public function del($mensalista_id = NULL) {
        ...
        if ($this->core_model->get_by_id('mensalidades', array('mensalidade_mensalista_id' => $mensalista_id))) {
            $this->session->set_flashdata('error', 'Não é possível excluir esse mensalista pois o mesmo está atrelado á <i class="fas fa-hand-holding-usd"></i> Mensalidades');
            redirect($this->router->fetch_class());
        }
